...
(defn cx [frame]
  (let [bg-colors [ (:dark-dark-blue water-colors)]; (:midnight earth-colors)
    ...
    (list
      ...
      ;;;;;;;;;;;;;;;;;;;;;;;;;;;
      ;
      ;[:text {:x (* 0.5 @width) :y (* 0.45 @height)
      ;        :style {:font-size 90
      ...
      ; "be aware"]
      ;
      ;[:text {:x (* 0.5 @width) :y (* 0.55 @height)
      ;        :style {:font-size 90
      ;                :font-family "helvetica"
      ;                :fill white
      ;                :text-anchor "middle"}}
      ; "flashy visuals on the horizon ..."])))
      ...
      ;                :fill white
      ;                :text-anchor "middle"}}])))
       ;"\uD83D\uDC97 time for gratitude shares \uD83D\uDC97"
       ;"thank you Sarah (@superSGP) for la-habra"
       ;"if curious, check-out sarahgp/la-habra/"
       ;"big gratitude for the Clojure community"
       ;"and everyone who helped with ClojureD"
       ;"\uD83D\uDC97 Martin and Matthias for your support \uD83D\uDC97"
       ;"\uD83D\uDC97 Fabrizio for being awesome \uD83D\uDC97"
       ;"check out CASSALPOPOLO on soundcloud"
       ;"thank you to myself for doing this"
